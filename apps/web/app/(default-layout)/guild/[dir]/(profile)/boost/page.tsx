...
import { GuildBoostPage } from '~pages/guild/ui/(by-dir)/segments/boost/perks';
...
export const generateMetadata = fallbackDefaultMetadata<{ dir: string }>(
  withMetadataCache(async (props) => {
    ...
    return generateNextMetadata({
      ...
      canonical: Routing.Club.clubByDir({ params: { dir, tab: 'boost' } }),
    });
  }, 'guild-boost')
);
...
export default async (props: NextPageParams<{ dir: string }>) => {
  ...
  const boostInAvailableSegments = GuildByDirProfileAbilities.boost({ role });
  if (!boostInAvailableSegments) forbidden();

  return <GuildBoostPage dir={dir} clubId={club.id} />;
};
