...
import { VoiceoverContent } from '~pages/(title)/title-detail/ui/title-tabs/voiceover-content';
...
export const generateMetadata = fallbackDefaultMetadata<TitleDetailPageParams>(async (props) => {
  ...
  return await withMetadataCache(async () => {
    ...
    const t = await getTranslations(`pages.title-voiceover.meta.${siteConfig.contentType}`);
    ...
    return generateNextMetadata({
      ...
      canonical: Routing.Title.detail({
        params: { dir, content: siteConfig.contentType, tab: TitleDetailPageTabs.VOICEOVER },
      }),
      // index: shouldIndex,
      // follow: shouldIndex,
    });
  }, `title-voiceover-${dir}`)(props);
});

export default async function VoiceoverPage() {
  return (
    <Suspense fallback={null}>
      <VoiceoverContent {...TestProps.id(`title-tab-content-${TitleDetailPageTabs.VOICEOVER}`)} />
    </Suspense>
  );
}
