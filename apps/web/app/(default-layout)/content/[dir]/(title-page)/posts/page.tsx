...
import { PostsContent } from '~pages/(title)/title-detail/ui/title-tabs/posts-content';
...
export const generateMetadata = fallbackDefaultMetadata<TitleDetailPageParams>(async (props) => {
  ...
  return await withMetadataCache(async () => {
    ...
    const t = await getTranslations(`pages.title-posts.meta.${siteConfig.contentType}`);
    ...
    return generateNextMetadata({
      ...
      canonical: Routing.Title.detail({
        params: { dir, content: siteConfig.contentType, tab: TitleDetailPageTabs.POSTS },
      }),
      // index: shouldIndex,
      // follow: shouldIndex,
    });
  }, `title-posts-${dir}`)(props);
});

export default async function PostsPage() {
  return (
    <Suspense fallback={null}>
      <PostsContent {...TestProps.id(`title-tab-content-${TitleDetailPageTabs.POSTS}`)} />
    </Suspense>
  );
}
