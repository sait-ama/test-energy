...
import { MomentsContent } from '~pages/(title)/title-detail/ui/title-tabs/moments-content';
...
export const generateMetadata = fallbackDefaultMetadata<TitleDetailPageParams>(async (props) => {
  ...
  return await withMetadataCache(async () => {
    ...
    const t = await getTranslations(`pages.title-moments.meta.${siteConfig.contentType}`);
    ...
    return generateNextMetadata({
      title: t('title', {
        ...
        count_moments: title.count_moments,
      }),
      ...
      canonical: Routing.Title.detail({
        params: { dir, content: siteConfig.contentType, tab: TitleDetailPageTabs.MOMENTS },
      }),
      // index: shouldIndex,
      // follow: shouldIndex,
    });
  }, `title-moments-${dir}`)(props);
});

export default async function MomentsPage() {
  return (
    <Suspense fallback={null}>
      <MomentsContent {...TestProps.id(`title-tab-content-${TitleDetailPageTabs.MOMENTS}`)} />
    </Suspense>
  );
}
