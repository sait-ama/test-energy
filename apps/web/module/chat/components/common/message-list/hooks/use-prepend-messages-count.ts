...
export function usePrependedMessagesCount(messages: ChatMessage[], hasDateSeparator: boolean) {
  const firstRealMessageIndex = hasDateSeparator ? 1 : 0;
  const firstMessageOnFirstLoadedPage = useRef<ChatMessage>(undefined);
  const previousFirstMessageOnFirstLoadedPage = useRef<ChatMessage>(undefined);
  ...
  const numItemsPrepended = useMemo(() => {
    ...
    const noNewMessages =
      currentFirstMessage?.uuid === previousFirstMessageOnFirstLoadedPage.current?.uuid;
    ...
    for (
      ...
      const messageIsFirstOnFirstLoadedPage =
        messages[prependedMessageCount]?.uuid === firstMessageOnFirstLoadedPage.current?.uuid;
