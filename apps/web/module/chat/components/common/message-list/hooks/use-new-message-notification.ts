...
import type { ChatMessage } from 'module/chat/context';
...
export function useNewMessageNotification(
  ...
  useEffect(() => {
    ...
    lastMessageId.current = lastMessage.uuid || ''; // update last message id

    /* do nothing if new messages are loaded from top(loadMore)  */
    if (lastMessage.uuid === prevMessageId) return;
    ...
    if (lastMessage.user_id !== currentUserId && didMount.current) {
