...
import { TextareaSuggestions } from './textarea-suggestions';
import {
...
export class TextareaAutocomplete extends React.Component {
  static defaultProps = {
  ...
  _onSelect = (newToken) => {
    const { closeCommandsList, closeMentionsList, onChange, showCommandsList, showMentionsList } =
      this.props;
  ...
  getTriggerProps = () => {
    ...
    if ((showCommandsList && trigger['/']) || (showMentionsList && trigger['@'])) {
      ...
      triggerProps.component = showCommandsList ? CommandItem : UserSuggestionItem;
  ...
  renderSuggestionListContainer() {
    ...
    const isEmoji = triggerProps.currentTrigger === ':';

    return (
      <div
        className={cn(
          'bg-background absolute right-0 bottom-[100%] left-0 border-t pb-2',
          isEmoji && 'flex flex-row px-0 pb-0',
          dropdownClassName
        )}
      ...
      >
        <TextareaSuggestions
          className={cn(listClassName, isEmoji && 'flex flex-row px-0')}
          dropdownScroll={this._dropdownScroll}
          itemClassName={cn('flex flex-row py-2 px-5', itemClassName)}
          itemStyle={itemStyle}
      ...
      <>
        ...
        >
          <Textarea
            ...
            {...this._cleanUpProps()}
            className={cn('resize-none text-[16px]', className)}
            maxRows={maxRows}
