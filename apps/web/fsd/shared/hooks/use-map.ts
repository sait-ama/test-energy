 ...
 * @template V - The type of values in the map.
 */
export type UseMapReturn<K, V> = [Omit<Map<K, V>, 'set' | 'clear' | 'delete'>, UseMapActions<K, V>];
...
export function useMap<K, V>(initialState: MapOrEntries<K, V> = new Map()): UseMapReturn<K, V> {
  ...
  const actions: UseMapActions<K, V> = {
    set: useCallback((key, valueOrFunc) => {
      setMap((prev) => {
        const copy = new Map(prev);
        const newValue =
          typeof valueOrFunc === 'function' ? valueOrFunc(copy.get(key)) : valueOrFunc;
