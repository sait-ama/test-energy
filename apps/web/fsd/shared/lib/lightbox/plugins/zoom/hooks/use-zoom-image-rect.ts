import { ContainerRect } from '~shared/hooks/use-container-rect';
...
import { isImageFitCover, isImageSlide, round } from '~shared/lib/lightbox/utils';
    ...
    if (isImageSlide(slide)) {
      ...
      if (width > 0 && height > 0 && slideRect.width > 0 && slideRect.height > 0) {
        ...
        imageRect = cover
          ...
          : {
              width: Math.round(
                Math.min(slideRect.width, (slideRect.height / height) * width, width)
              ),
              height: Math.round(
                Math.min(slideRect.height, (slideRect.width / width) * height, height)
              ),
            };
      }
    }
