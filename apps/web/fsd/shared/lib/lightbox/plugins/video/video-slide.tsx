...
import { LightboxProps, SlideVideo } from '../../types';
import { cssClass } from '../../utils';
import { useVideoProps } from './props';
...
export function VideoSlide({ slide, offset }: VideoSlideProps) {
  ...
  const scaleWidthAndHeight = () => {
    ...
    if (width && height && containerRect) {
      ...
      const elementWidth = widthBound
        ? containerRect.width
        : Math.round((containerRect.height / height) * width);
          ...
          {containerRect && (
            ...
            >
              {sources.map(({ src, type, media }) => (
                <source
                  key={[src, type, media].filter(Boolean).join('|')}
                  src={src}
                  type={type}
                  media={media}
                />
