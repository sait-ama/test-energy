...
import { UseSensors } from '~shared/lib/lightbox/hooks/use-sensors';
import { useTimeouts } from '~shared/lib/lightbox/stores/timeouts';

import { EVENT_ON_WHEEL } from '../../consts';
...
export function useWheelSwipe<T extends Element = Element>(
  ...
  const onWheel = useEventCallback((event: React.WheelEvent) => {
    ...
    if (swipeState === SwipeState.NONE) {
      ...
      newSwipeOffset =
        Math.min(Math.abs(newSwipeOffset), containerWidth) * Math.sign(newSwipeOffset);
