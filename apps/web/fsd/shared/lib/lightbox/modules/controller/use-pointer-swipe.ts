  ...
  const onPointerMove = useEventCallback((event: React.PointerEvent) => {
    ...
    if (pointer) {
      ...
      if (event.buttons === 0) {
        ...
        // 1) active pointer must have been released while over an opaque element
        // 2) out-of-order onpointermove / onpointerup stores in Chrome
      ...
      if (activePointer.current === undefined) {
        ...
        if (
          ...
          exceedsPullThreshold(deltaY, SWIPE_THRESHOLD)
        ) {
