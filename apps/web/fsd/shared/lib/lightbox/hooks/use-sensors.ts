...
export type RegisterSensors<T extends Element> = Required<
  Pick<React.HTMLAttributes<T>, PointerEventType>
> &
  ...
  const registerSensors = React.useMemo(
    () => ({
      onPointerDown: (event: React.PointerEvent<T>) =>
        notifySubscribers(EVENT_ON_POINTER_DOWN, event),
      onPointerMove: (event: React.PointerEvent<T>) =>
        notifySubscribers(EVENT_ON_POINTER_MOVE, event),
      ...
      onPointerLeave: (event: React.PointerEvent<T>) =>
        notifySubscribers(EVENT_ON_POINTER_LEAVE, event),
      onPointerCancel: (event: React.PointerEvent<T>) =>
        notifySubscribers(EVENT_ON_POINTER_CANCEL, event),
