...
export type EventCallback<T extends Topic> = (
  ...args: Event<T> extends void ? [] : [event: Event<T>]
) => void;
...
export function EventsProvider({ children }: React.PropsWithChildren) {
  ...
  const context = React.useMemo<EventsContextType>(() => {
    ...
    const publish = <T extends Topic>(
      ...[topic, event]: Event<T> extends void ? [T] : [T, Event<T>]
    ) => {
      // most likely culprit - https://github.com/microsoft/TypeScript/issues/29131
      subscriptions[topic]?.forEach((callback) =>
        (callback as unknown as (event?: Event<T>) => void)(event)
      );
    };
