...
export const EditTitleValidator = BaseTitleValidator;
export const PublishersValidator = z.object({
  publishers: z.object({
    type: z.string(),
    branch_id: z.number(),
    publishers: z.array(z.number()).optional(),
    user_message: z.string().optional(),
  }),
});
export const CreatorsValidator = z.object({
  creators: z.object({
    creators: z.array(z.number()).optional(),
    user_message: z.string().optional(),
  }),
...
export const AdditionalValidator = z
  .object({
    additional: z.object({
  ...
  .superRefine((data, ctx) => {
    if (!data.additional.links.find((link) => link.type === 'original_link')) {
      ctx.addIssue({
        ...
        message: 'Ссылка на оригинал обязательна. Укажите в разделе "Ссылки"',
      });
    }
  });
export type EditTitleSchema = z.infer<typeof EditTitleValidator>;
